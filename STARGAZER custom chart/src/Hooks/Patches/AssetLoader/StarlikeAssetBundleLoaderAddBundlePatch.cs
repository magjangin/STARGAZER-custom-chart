        ...
        private static class StarlikeAssetBundleLoaderAddBundlePatch
        {
            ...
            private static void Prefix(MethodBase __originalMethod, object? __instance, object[]? __args)
            {
                try
                {
                    ...
                    MelonLogger.Msg($"[AssetLoader][AddBundle][PRE] key={key}, bundleType={bundleType}, instance={instanceType}");
                }
                catch (Exception ex)
                {
                    MelonLogger.Warning($"[AssetLoader][AddBundle][PRE] logging failed: {ex.GetType().Name}: {ex.Message}");
                }
            }
        }
    }
